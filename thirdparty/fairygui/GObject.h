...
public:
    ...
    const ax::ValueMap& getCustomData() const { return _customData; }
    void setCustomData(const ax::ValueMap& value) { _customData = value; }
    void addCustomData(const std::string& key, const ax::Value& value) {
        if (_customData.count(key)) {
            _customData.erase(key);
        }
